...
C       ADDED
        IMPLICIT NONE
        INTEGER X, Y
        DIMENSION NEST, NEST1, DIS
C REPLACED TAPE INPUT WITH FILE ACCESS
C        (INPU,OUTP,TAPE1 = INPU, TAPE2 = OUTP  )
        OPEN (INPU, FILE="INPU.txt", STATUS='OLD')
        OPEN (OUTP, FILE="OUTP.txt", STATUS='OLD')
C       INTEGER X(1000),Y(1000)
        X = 1000
        Y = 1000
C        DIMENSION NEST(1000),NEST1(1000),DIS(1000)
        NEST = 1000
        NEST1 = 1000
        DIS = 1000
